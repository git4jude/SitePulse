...
export default function Navbar() {
    ...
    return (
        <div className="fixed top-4 inset-x-0 z-50 max-w-7xl mx-auto px-4 sm:px-6">
            <nav className="relative overflow-hidden rounded-2xl border border-border shadow-lg bg-background">
                            ...
                            <div className="hidden md:flex items-center gap-1">
                                {navLinks.map((link) => (
                                    <Link
                                        key={link.path}
                                        to={link.path}
                                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all ${isActive(link.path) ? "bg-[#9333EA]/10 text-[#9333EA] font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted/80"}`}
                                    >
                                        {link.icon}
                                        {link.label}
                                    </Link>
                            ...
                            {user ? (
                                <>
                                    <div className="flex items-center gap-2 px-2 py-1.5 rounded-full border border-border bg-card text-sm">
                                        ...
                                        <span className="px-2 py-0.5 rounded-full text-[10px] font-medium uppercase bg-[#9333EA]/10 border border-[#9333EA]/15 text-[#9333EA]">{user.plan}</span>
                                    </div>
                ...
                {/* Mobile menu */}
                {mobileOpen && (
                    <div className="md:hidden border-t border-border bg-background origin-top">
                                    ...
                                    <div className="py-2 space-y-1">
                                        {navLinks.map((link) => (
                                            <Link
                                                ...
                                                className={`flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium transition-all ${isActive(link.path) ? "bg-[#9333EA]/10 text-[#9333EA]" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}
                                            >
                                                {link.icon}
                                                {link.label}
                                            </Link>
