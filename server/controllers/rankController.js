...
export const addKeyword = async (req, res) => {
  try {
    ...
    if(!keyword || !url){
      return res.status(400).json({success: false, message: "Keyword and url required"})
    }
...
export const getKeyword = async (req, res) => {
  try {
    ...
    res.json({ success: true, keyword: tracking })
  } catch (error) {
...
export const toggleKeyword = async (req, res) => {
  try {
    ...
    res.json({ success: true, active: tracking.active })
  } catch (error) {
